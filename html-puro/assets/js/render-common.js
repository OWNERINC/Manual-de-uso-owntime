...
function renderHero(tipologia) {
  document.getElementById('hero').innerHTML = `
    <div class="hero__bg" style="--hero-image: url('${tipologia.hero.image}')"></div>
...
function renderOrientacoesGerais(common) {
  ...
  const cards = items.map(item => `
    <button class="bs-trigger orientacao-card" data-bs-target="sheet-orientacao-${item.id}" aria-label="${item.title}">
      <span class="orientacao-card__icon-wrap" aria-hidden="true">
        <i data-lucide="${item.icon}"></i>
      </span>
      <span class="orientacao-card__title">${item.title}</span>
    </button>
  `).join('');
  ...
  document.getElementById('orientacoes-gerais').innerHTML = `
    <div class="container section-pad">
      ...
      <div class="orientacoes__cards">${cards}</div>
    </div>
  `;
